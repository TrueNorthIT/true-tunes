...
class SonosGroupManager {
  ...
  async AddToQueue(uri, index) {
    if (this.coordinator) {
      await this.coordinator.AddUriToQueue(uri, index);
      return 'Added';
    }
  }
  async PlaySongNow(uri) {
    if (this.coordinator) {
  ...
  async ReorderTracksInQueue(startingIndex, numberOfTracks, insertBefore) {
    if (this.coordinator) {
...
electron__WEBPACK_IMPORTED_MODULE_1__.ipcMain.handle('search', async (event, searchTerm, searchType, service, resultCount) => {
...
electron__WEBPACK_IMPORTED_MODULE_1__.ipcMain.handle('addToQueue', async (event, uri, index) => {
  await sonosManager.AddToQueue(uri, index);
  return 'Added';
});
electron__WEBPACK_IMPORTED_MODULE_1__.ipcMain.handle('reorderTracksInQueue', async (event, startingIndex, numberOfTracks, insertBefore) => {
  await sonosManager.ReorderTracksInQueue(startingIndex, numberOfTracks, insertBefore);
  return 'Reordered';
});
electron__WEBPACK_IMPORTED_MODULE_1__.ipcMain.handle('playSongNow', async (event, uri) => {

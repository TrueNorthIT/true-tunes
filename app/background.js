...
/* harmony import */ var _SonosGroupManager__WEBPACK_IMPORTED_MODULE_6__ = __webpack_require__(/*! ./SonosGroupManager */ "./main/SonosGroupManager.ts");
/* harmony import */ var _renderer_msalConfig__WEBPACK_IMPORTED_MODULE_7__ = __webpack_require__(/*! ../renderer/msalConfig */ "./renderer/msalConfig.ts");
/* harmony import */ var _azure_msal_node__WEBPACK_IMPORTED_MODULE_8__ = __webpack_require__(/*! @azure/msal-node */ "@azure/msal-node");
/* harmony import */ var _azure_msal_node__WEBPACK_IMPORTED_MODULE_8___default = /*#__PURE__*/__webpack_require__.n(_azure_msal_node__WEBPACK_IMPORTED_MODULE_8__);
...
let server;
const msalInstance = new _azure_msal_node__WEBPACK_IMPORTED_MODULE_8__.PublicClientApplication(_renderer_msalConfig__WEBPACK_IMPORTED_MODULE_7__.msalConfig);
electron__WEBPACK_IMPORTED_MODULE_3__.ipcMain.handle('auth-login', async () => {
  return new (_babel_runtime_corejs3_core_js_stable_promise__WEBPACK_IMPORTED_MODULE_0___default())(async (resolve, reject) => {
    if (server) {
      server.close();
    }
    server = http__WEBPACK_IMPORTED_MODULE_9___default().createServer(async (req, res) => {
      const queryObject = url__WEBPACK_IMPORTED_MODULE_10___default().parse(req.url, true).query;

      // Ensure the queryObject.code is a string and not an array
      const authorizationCode = _babel_runtime_corejs3_core_js_stable_array_is_array__WEBPACK_IMPORTED_MODULE_1___default()(queryObject.code) ? queryObject.code[0] : queryObject.code;
      if (authorizationCode) {
        try {
          // Exchange the authorization code for a token (MSAL automatically handles PKCE)
          const tokenResponse = await msalInstance.acquireTokenByCode({
            code: authorizationCode,
            scopes: _renderer_msalConfig__WEBPACK_IMPORTED_MODULE_7__.msalConfig.scopes,
            redirectUri: _renderer_msalConfig__WEBPACK_IMPORTED_MODULE_7__.msalConfig.auth.redirectUri
          });
    ...
    server.listen(3000, () => {
      ...
      msalInstance.getAuthCodeUrl({
        ...
        electron__WEBPACK_IMPORTED_MODULE_3__.shell.openExternal(authUrl); // Opens the system default browser for login
      }).catch(error => {
...
electron__WEBPACK_IMPORTED_MODULE_3__.ipcMain.handle('get-user-photo', async (event, accessToken) => {
  try {
...
const sonosManager = new _SonosGroupManager__WEBPACK_IMPORTED_MODULE_6__.SonosGroupManager();
electron__WEBPACK_IMPORTED_MODULE_3__.ipcMain.handle('connect', async (event, groupName) => {
  await sonosManager.Connect(groupName);
  return 'Connected';
});
electron__WEBPACK_IMPORTED_MODULE_3__.ipcMain.handle('search', async (event, term, searchType) => {
  const result = await sonosManager.Search(term, searchType, 9);
  return result;
});
electron__WEBPACK_IMPORTED_MODULE_3__.ipcMain.handle('getQueue', async event => {
  const result = await sonosManager.GetQueue();
  return result;
});
electron__WEBPACK_IMPORTED_MODULE_3__.ipcMain.handle('connectToServices', async event => {
  await sonosManager.ConnectToServices();
  return 'Connected';
});
electron__WEBPACK_IMPORTED_MODULE_3__.ipcMain.handle('getConnectionStatus', async event => {
  const result = await sonosManager.GetConnectionStatus();
  return result;
});
electron__WEBPACK_IMPORTED_MODULE_3__.ipcMain.handle('getMusicServiceRootPage', async (event, service) => {
  const result = await sonosManager.GetRootPage(service);
  return result;
});
electron__WEBPACK_IMPORTED_MODULE_3__.ipcMain.handle('getMetadata', async (event, service, id) => {
  const result = await sonosManager.GetMetadata(service, id);
  return result;
});
electron__WEBPACK_IMPORTED_MODULE_3__.ipcMain.handle('next', async event => {
  await sonosManager.Next();
  return 'Next';
});
electron__WEBPACK_IMPORTED_MODULE_3__.ipcMain.handle('previous', async event => {
  await sonosManager.Previous();
  return 'Previous';
});
electron__WEBPACK_IMPORTED_MODULE_3__.ipcMain.handle('togglePlayback', async event => {
  await sonosManager.TogglePlayback();
  return 'Toggled';
});
electron__WEBPACK_IMPORTED_MODULE_3__.ipcMain.handle('getPlaybackState', async event => {
  const result = await sonosManager.GetPlaybackState();
  return result;
});
electron__WEBPACK_IMPORTED_MODULE_3__.ipcMain.handle('getVolume', async event => {
  const result = await sonosManager.GetVolume();
  return result;
});
electron__WEBPACK_IMPORTED_MODULE_3__.ipcMain.handle('setVolume', async (event, volume) => {
  await sonosManager.SetVolume(volume);
  return 'Volume set';
});
electron__WEBPACK_IMPORTED_MODULE_3__.ipcMain.handle('jumpToPointInQueue', async (event, index) => {
